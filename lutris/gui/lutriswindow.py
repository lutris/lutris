...
from lutris.gui.widgets.utils import get_widget_children, load_icon_theme, open_uri, set_cursor_by_name
...
class LutrisWindow(Gtk.ApplicationWindow, DialogLaunchUIDelegate, DialogInstallUIDelegate):  # type:ignore[misc]
    ...
    search_filters_button: Gtk.MenuButton = Gtk.Template.Child()
    ...
    login_notification_revealer: Gtk.Revealer = Gtk.Template.Child()
    lutris_log_in_label: Gtk.Label = Gtk.Template.Child()
    version_notification_revealer: Gtk.Revealer = Gtk.Template.Child()
    version_notification_label: Gtk.Label = Gtk.Template.Child()
    ...
    def __init__(self, application=None, **kwargs) -> None:
        ...
        lutris_icon = Gtk.Image.new_from_icon_name("net.lutris.Lutris")
        lutris_icon.set_margin_end(3)
        ...
        self.game_bar = None
        self.revealer_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL)
    ...
    def update_busy_cursor(self):
        """Applies the 'progress' cursor to this window if Lutris is busy."""
    ...
    def on_drag_drop(self, drop_target, value, x, y):
        """Handler for drop event (GTK 4 DropTarget "drop" signal)"""
    ...
    def update_revealer(self, games=None):
        if games is not None:  # games can be an empty list!
            ...
            if len(games) == 1 and games[0]:
                ...
                self.game_bar.set_hexpand(True)
            ...
            pass
        if self.revealer_box.get_first_child():
    ...
    def save_window_state(self):
        """Saves the window's size and state as settings.
    ...
    def restore_window_position(self):
        """Window positioning is managed by the compositor in GTK 4;
        applications can no longer place their own windows."""
        pass
    ...
    def on_resize(self, widget, *_args):
        ...
        if not self.maximized:
            self.window_size = (width, height)
        self.search_entry.set_size_request(min(max(50, width - 470), 800), -1)
    ...
    def on_window_delete(self, *_args):
        ...
        if app.has_tray_icon():
            self.set_visible(False)
            return True
        # Window is actually closing — stop cancellable threads now, since
        # `destroy` may not fire when the last window closes (app quits first).
