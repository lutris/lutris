...
class Runner:  # pylint: disable=too-many-public-methods
    ...
    async def run(self, ui_delegate):
        """Run the runner alone."""
        if not self.runnable_alone:
            return
        if not self.is_installed():
            if not await self.install_dialog(ui_delegate):
    ...
    async def install_dialog(self, ui_delegate):
        """Ask the user if they want to install the runner.

        Return success of runner installation.
        """

        if await ui_delegate.show_install_yesno_inquiry(
            ...
            if hasattr(self, "get_version"):
                ...
                if not await self.install(ui_delegate, version=version):
                    return False
            else:
                if not await self.install(ui_delegate):
                    return False
    ...
    async def install(self, install_ui_delegate, version=None):
        ...
        if self.download_url:
            ...
            return await self.download_and_extract(self.download_url, **opts)
        ...
        return await self.download_and_extract(runner_version_info["url"], **opts)

    async def download_and_extract(self, url, dest=None, **opts):
        ...
        if not await install_ui_delegate.download_install_file(url, runner_archive):
            return False
