...
class pico8(Runner):
    ...
    def engine_path(self):
        ...
        if not engine.lower().endswith('.js') and not os.path.exists(engine):
            engine = os.path.join(settings.RUNNER_DIR, 'pico8/web/engines', self.runner_config.get('engine') + '.js')
        return engine
    ...
    def prelaunch(self):
        ...
        if self.is_native or not self.game_config.get('main_file').startswith('http'):
            if not os.path.exists(self.game_config.get('main_file')) and (self.game_config.get('main_file').isdigit() or self.game_config.get('main_file').startswith('http')):
