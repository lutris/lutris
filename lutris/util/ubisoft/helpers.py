...
def _get_registry_value_from_path(registry_path: str, key: str) -> str:
    registry = WineRegistry()
    return registry.query(registry_path, key)


def _return_local_game_path_from_special_registry(special_registry_path: Optional[str]) -> str:
...
def _return_local_game_path(launch_id: str) -> str:
...
def get_local_game_path(special_registry_path: str, launch_id: str) -> Optional[str]:
...
async def get_size_at_path(start_path: str) -> int:
    total_size = 0
...
def _is_file_at_path(path: str, file: str) -> bool:
    if os.path.isdir(path):
        return os.path.isfile(os.path.join(path, file))
    return False


def _read_status_from_state_file(game_path: str) -> str:
    try:
...
def get_game_installed_status(path: str, exe: Optional[str] = None, special_registry_path: Optional[str] = None) -> str:
...
def get_ubisoft_registry(prefix: WinePrefixManager, fullpath: str) -> str:
...
def convert_ubisoft_key(key_path: str) -> str:
