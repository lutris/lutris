...
class GOGService(OnlineService):
    ...
    def get_game_dlcs(self, product_id: str) -> list[dict]:
        ...
        if game_details["dlcs"]["products"].size() > self.MAX_PRODUCT_IDS_PER_REQUEST:
            # Get product ids only form json
            product_ids = []
            for product in game_details["dlcs"]["products"]:
                product_ids.append(product["id"])
            # Get number of requests by batches of 50
            if len(product_ids) % self.MAX_PRODUCT_IDS_PER_REQUEST <= 0:
                num_requests = len(product_ids) / self.MAX_PRODUCT_IDS_PER_REQUEST
            else:
                num_requests = (len(product_ids) / self.MAX_PRODUCT_IDS_PER_REQUEST) + 1
            # Call make_api_request for each array
            for i in range(int(num_requests)):
                start_index = self.MAX_PRODUCT_IDS_PER_REQUEST * i
                # Check if start index is out of bounds
                if start_index >= len(product_ids):
                    break
                # Check if end index exceeds array length
                if (start_index + self.MAX_PRODUCT_IDS_PER_REQUEST - 1) >= len(product_ids):
                    # Set end index to -1 to close loop
                    end_index = -1
                else:
                    # Create index for array of 50 items [i.e. 0 to 49]
                    end_index = start_index + self.MAX_PRODUCT_IDS_PER_REQUEST - 1
