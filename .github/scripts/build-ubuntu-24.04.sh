...
# script overrides the build generic build process for Ubuntu 24.04 (Noble) and
...
# First run the standard build process for Noble.
# shellcheck source=./build-ubuntu-generic.sh
source "${CODEBASE_ROOT}/.github/scripts/build-ubuntu-generic.sh"
echo "::endgroup::"

# Rerun the build process for all codenames in the NOBLE_BUILDS env
...
# our Noble runner.
if [[ -n ${NOBLE_BUILDS} ]]; then
    for OS_CODENAME in ${NOBLE_BUILDS}; do
        if ! distro-info --series "${OS_CODENAME}"; then
            echo "Bad NOBLE_BUILDS codename '${OS_CODENAME}' provided.  Skipping this build."
        else
